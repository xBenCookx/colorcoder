...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Function to get complementary color from backend
    async function getComplementaryColor(color) {
        try {
            ...
            if (!response.ok) {
                throw new Error('Failed to get complementary color');
            }

            const data = await response.json();
            return data.complementaryColor;
        } catch (error) {
    ...
    applyColorButton.addEventListener('click', async () => {
        if (selectedColor) {
            const complementaryColor = await getComplementaryColor(selectedColor);
